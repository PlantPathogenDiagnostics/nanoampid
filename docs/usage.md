# PlantPathogenDiagnostics/nanoampid: Usage
...
```bash
nextflow run PlantPathogenDiagnostics/nanoampid --input ./samplesheet.csv --outdir ./results --reference /path/to/ref_db.fasta -profile docker
```
...
```bash
nextflow run PlantPathogenDiagnostics/nanoampid -profile docker -params-file params.yaml
```
...
First, go to the [PlantPathogenDiagnostics/nanoampid releases page](https://github.com/PlantPathogenDiagnostics/nanoampid/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
