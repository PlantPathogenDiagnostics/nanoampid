process FILTLONG {
    ...
    def prefix = "${longreads}".replaceAll(/\.fastq(?:\.gz)?$|\.fq(?:\.gz)?$/,'')
    def short_reads = !shortreads ? "" : meta.single_end ? "-1 $shortreads" : "-1 ${shortreads[0]} -2 ${shortreads[1]}"
    if ("$longreads" == "${prefix}.fq.gz") error "Longread FASTQ input and output names are the same, set prefix in module configuration to disambiguate!"
    """
    filtlong \\
        $short_reads \\
        $args \\
